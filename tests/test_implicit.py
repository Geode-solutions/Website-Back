...
def test_step1(client):
    ...
    constraints_stupid = (
        '[{ "x": "5", "y": "toto", "z": "9.5", "value": "0", "weight": "10" }]'
    )
    response = client.post(
        f"{base_route}/step1",
        data={
...
def test_step2(client):
    ...
    response = client.post(
        f"{base_route}/step2", data={"axis": axis, "direction": direction}
    )
    assert response.status_code == 200
    viewable_file_name = response.json['viewable_file_name']
    id = response.json['id']
    ...
    # Test without axis
    response = client.post(f"{base_route}/step2", data={"direction": direction})
    assert response.status_code == 400
    error_description = response.json["description"]
    ...
    axis_stupid = "Toto"
    response = client.post(
        f"{base_route}/step2", data={"axis": axis_stupid, "direction": direction}
    )
    assert response.status_code == 400
    error_description = response.json["description"]
    assert (
        error_description == "Invalid data format for the 'axis' or 'metric' variables"
    )

    # Test with stupid direction value
    direction_stupid = "Toto"
    response = client.post(
        f"{base_route}/step2", data={"axis": axis, "direction": direction_stupid}
    )
    assert response.status_code == 400
    error_description = response.json["description"]
    assert (
